...
import { useStore, handleWorkflowEvent, stopWorkflowExecution, pauseWorkflowExecution, resumeWorkflowExecution } from '../../store/useStore';
                ...
                {executionInstance && executionInstance.status !== 'completed' && executionInstance.status !== 'failed' && (
                  <>
                    {executionInstance.status === 'running' && (
                      <button onClick={pauseWorkflowExecution} className="px-3 py-2 bg-yellow-600 hover:bg-yellow-700 text-white font-bold rounded-lg">
                        ⏸ {isZh ? '暂停' : 'Pause'}
                      </button>
                    )}
                    {executionInstance.status === 'paused' && (
                      <button onClick={resumeWorkflowExecution} className="px-3 py-2 bg-green-600 hover:bg-green-700 text-white font-bold rounded-lg">
                        ▶ {isZh ? '继续' : 'Resume'}
                      </button>
                    )}
                    ...
                    <button
                      onClick={async () => {
                        const confirmed = window.confirm(isZh ? '确定要停止工作流吗？停止后不可恢复。' : 'Stop workflow? This cannot be undone.');
                        if (confirmed) {
